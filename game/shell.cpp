 ...
 * - Any negative: Error
 */
NATIVE_API int Shell_PickFolder(wchar_t *title, wchar_t *startingDirectory,
  ...
  *pickedFolder = nullptr;
  ...
  if (startingDirectory) {
    ...
    if (SUCCEEDED(SHCreateItemFromParsingName(startingDirectory, nullptr,
                                              IID_PPV_ARGS(&preselectedFolder)))) {
  ...
  *pickedFolder = copyString(fullPath);
...
NATIVE_API ApiBool Shell_ShowPrompt(bool errorIcon, const wchar_t *promptTitle,
                                    const wchar_t *promptEmphasized,
                                    const wchar_t *promptDetailed) {
  int button = 0;
  TaskDialog(NULL, NULL, promptTitle, promptEmphasized, promptDetailed,
             TDCBF_OK_BUTTON | TDCBF_CLOSE_BUTTON, errorIcon ? TD_ERROR_ICON : TD_INFORMATION_ICON,
             &button);
...
NATIVE_API void Shell_ShowMessage(bool errorIcon, const wchar_t *promptTitle,
                                  const wchar_t *promptEmphasized, const wchar_t *promptDetailed) {
  int button = 0;
  TaskDialog(NULL, NULL, promptTitle, promptEmphasized, promptDetailed, TDCBF_OK_BUTTON,
             errorIcon ? TD_ERROR_ICON : TD_INFORMATION_ICON, &button);
}
