...
struct VideoContext {
  ...
  std::unique_ptr<StreamContext> create_stream_context(AVCodecParameters* codec_params, int index) {
  ...
  void play(VideoPlayer::OnVideoFrame onVideoFrame, VideoPlayer::OnAudioSamples onAudioSamples,
            VideoPlayer::OnStop onStop);
  void stop();

 private:
  void run_decode_loop(VideoPlayer::OnVideoFrame onVideoFrame,
                       VideoPlayer::OnAudioSamples onAudioSamples, VideoPlayer::OnStop onStop);
};
...
bool VideoPlayer::open(const char* path) {
  ...
  for (int i = 0; i < video->format_ctx->nb_streams; i++) {
...
const std::string& VideoPlayer::error() const { return video->error; }
...
void VideoPlayer::stop() { video->stop(); }
