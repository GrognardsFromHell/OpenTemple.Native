...
namespace Qml.Net
{
    public static class Qml
    {
        ...
        public static int RegisterSingletonInstance(object singleton, string qmlName, string uri, int versionMajor = 1, int versionMinor = 0)
        {
            using var typeInfo = NetTypeManager.GetTypeInfo(singleton.GetType());
            using var objRef = NetReference.CreateForObject(singleton);

            return Interop.QQmlApplicationEngine.RegisterSingletonInstanceNet(typeInfo.Handle, uri, versionMajor,
                versionMinor, qmlName, objRef.Handle);
        }
    }
}