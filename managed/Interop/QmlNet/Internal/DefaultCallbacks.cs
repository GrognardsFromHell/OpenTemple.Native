        ...
        public void LoadTypeInfo(IntPtr t)
        {
            using (var type = new NetTypeInfo(t))
            {
                ...
                foreach (var propertyInfo in typeInfo.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
                {
                    ...
                    // Hypothesis: string Prop {get;} can be considered read-only
                    var isConstant = false;
                    if (!propertyInfo.CanWrite && notifySignal == null && propertyInfo.GetMethod != null)
                    {
                        var cga = propertyInfo.GetMethod.GetCustomAttribute<CompilerGeneratedAttribute>();
                        isConstant = cga != null;
                    }
