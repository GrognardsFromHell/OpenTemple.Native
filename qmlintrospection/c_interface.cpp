...
NATIVE_API char16_t *PropInfo_name(PropInfo &prop) { return copyString(prop.name); }
...
#pragma region QmlInfo Wrappers
...
NATIVE_API bool QmlInfo_addMetaClass(QmlInfo &qmlInfo, const char *metaClassName) {
  return qmlInfo.addMetaClass(metaClassName);
}

NATIVE_API bool QmlInfo_visitTypeLibrary(QmlInfo &qmlInfo, void (*visitor)(const TypeLibrary &)) {
...
NATIVE_API TypeInfoKind TypeInfo_kind(const TypeInfo &info) { return info.kind; }

NATIVE_API char16_t *TypeInfo_metaClassName(const TypeInfo &info) {
  return copyString(info.metaClassName);
}

NATIVE_API char16_t *TypeInfo_name(const TypeInfo &info) { return copyString(info.name); }

NATIVE_API char16_t *TypeInfo_qmlModule(const TypeInfo &info) { return copyString(info.qmlModule); }

NATIVE_API int TypeInfo_qmlMajorVersion(const TypeInfo &info) { return info.qmlMajorVersion; }

NATIVE_API int TypeInfo_qmlMinorVersion(const TypeInfo &info) { return info.qmlMinorVersion; }

NATIVE_API bool TypeInfo_qmlCreatable(const TypeInfo &info) { return info.qmlCreatable; }
...
NATIVE_API int TypeInfo_props_size(const TypeInfo &info) { return (int)info.props.size(); }

NATIVE_API const PropInfo *TypeInfo_props(const TypeInfo &info, int index) {
  return &info.props[index];
}

NATIVE_API int TypeInfo_signals_size(const TypeInfo &info) { return (int)info.signalInfos.size(); }

NATIVE_API const MethodInfo *TypeInfo_signals(const TypeInfo &info, int index) {
  return &info.signalInfos[index];
}

NATIVE_API int TypeInfo_methods_size(const TypeInfo &info) { return (int)info.methods.size(); }
...
NATIVE_API int TypeInfo_enums_size(const TypeInfo &info) { return (int)info.enums.size(); }

NATIVE_API const EnumInfo *TypeInfo_enums(const TypeInfo &info, int index) {
  return &info.enums[index];
}

NATIVE_API const TypeInfo *TypeInfo_parent(const TypeInfo &info) { return info.parent; }

#pragma endregion

#pragma region TypeLibrary Wrappers

NATIVE_API void TypeLibrary_visitTypes(TypeLibrary &library, void (*visitor)(const TypeInfo *)) {
...
NATIVE_API const TypeInfo *TypeRef_typeInfo(TypeRef &typeRef) { return typeRef.typeInfo; }

NATIVE_API char *TypeRef_enumName(TypeRef &typeRef) { return copyString(typeRef.enumName); }

NATIVE_API BuiltInType TypeRef_builtIn(TypeRef &typeRef) { return typeRef.builtIn; }

#pragma endregion

#pragma region EnumInfo Wrappers

NATIVE_API char16_t *EnumInfo_name(EnumInfo &info) { return copyString(info.name); }

NATIVE_API bool EnumInfo_flags(EnumInfo &info) { return info.flags; }

NATIVE_API BuiltInType EnumInfo_baseType(EnumInfo &info) { return info.baseType; }

NATIVE_API int EnumInfo_values_size(EnumInfo &info) { return (int)info.values.size(); }
...
NATIVE_API char16_t *EnumValueInfo_name(EnumValueInfo &info) { return copyString(info.name); }

NATIVE_API int EnumValueInfo_value(EnumValueInfo &info) { return info.value; }
