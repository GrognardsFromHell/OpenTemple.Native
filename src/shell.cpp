 ...
 * - Any negative: Error
 */
NATIVE_API int Shell_PickFolder(wchar_t *title, wchar_t *startingDirectory, WideStringResult *pickedFolder) {
    ...
    if (startingDirectory) {
        ...
        if (SUCCEEDED(SHCreateItemFromParsingName(startingDirectory, nullptr, IID_PPV_ARGS(&preselectedFolder)))) {
    ...
    *pickedFolder = new std::wstring(fullPath);
...
NATIVE_API ApiBool Shell_ShowPrompt(
        bool errorIcon,
...
NATIVE_API void Shell_ShowMessage(
        bool errorIcon,
        const wchar_t *promptTitle,
        const wchar_t *promptEmphasized,
        const wchar_t *promptDetailed
